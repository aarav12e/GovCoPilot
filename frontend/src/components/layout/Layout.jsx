  ...
  return (
    <div className="flex min-h-screen bg-bg">
      ...
      <div className="fixed top-0 left-0 w-[600px] h-[600px] rounded-full pointer-events-none z-0"
        style={{background:'radial-gradient(circle,rgba(255,153,51,0.04) 0%,transparent 70%)'}}/>
      <div className="fixed bottom-0 right-0 w-[500px] h-[500px] rounded-full pointer-events-none z-0"
        style={{background:'radial-gradient(circle,rgba(19,136,8,0.04) 0%,transparent 70%)'}}/>

      {/* Sidebar */}
      <aside className={`w-60 bg-surface border-r border-border1 flex flex-col fixed top-0 left-0 bottom-0 z-50 transition-transform duration-300 ${open?'translate-x-0':'-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="px-5 py-3.5 border-t border-border1">
      ...
      <div className="lg:ml-60 flex-1 flex flex-col min-h-screen relative z-10">
        <header className="bg-surface border-b border-border1 flex items-center justify-between px-6 sticky top-0 z-40 h-[60px]">
          <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <NavLink to="/chat"><button className="bg-saffron text-black rounded-lg px-3 py-1.5 text-xs font-display font-bold hover:bg-saffron2 transition-colors">💬 Ask AI</button></NavLink>
          </div>
        </header>

        <main className="flex-1 p-6 page-enter"><Outlet/></main>
      </div>
